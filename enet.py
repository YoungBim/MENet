...
#Now actually start building the network's Encoder
def ENetEncoder(inputs,
         batch_size,
         ...
         reuse=None,
         is_training=True,
         scope='Mult-ENet-Encoder'):
    '''
    The ENet-Encoder model for real-time feature encoding
    ...
    OUTPUTS:
    - feats(Tensor): a 4D Tensor output of shape [batch_size, image_height/8, image_width/8, 128] (i.e. the encoded features).
    '''
...
def ENetSegDecoder(Encoder,
         ...
         reuse=None,
         is_training=True,
         scope='Mult-ENet-SegDecoder'):
    '''
    The ENet model for real-time semantic segmentation!

    INPUTS:
    - inputs_shapes(List): a list of shapes for each of the encoder bottleneck specigying the associatd input shape
    - num_classes(int): an integer for the number of classes to predict. This will determine the final output channels as the answer.
    - batch_size(int): the batch size to explictly set the shape of the inputs in order for operations to work properly.
    - num_initial_blocks(int): the number of times to repeat the initial block.
    - stage_two_repeat(int): the number of times to repeat stage two in order to make the network deeper.
